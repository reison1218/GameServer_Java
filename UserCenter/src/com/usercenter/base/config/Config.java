...
public final class Config {
	...
	public final static boolean init(String configPost) {
		...
		String configPath = "/com/usercenter/config/config" + configPost + ".json";
		try {
			File file = null;
			if (SystemEnv.isProduction()) {
				...
				file = new File(Config.class.getResource(configPath).getFile());
			}
			if (!file.exists()) {
	...
	public final static boolean initNet() {
		try {
			...
			if (StringUtil.isNullOrEmpty(tempEngName)) {
	...
	public final static <T> T getConfig(String type, String key, Integer idx) {
		...
		Map<String, T> redisMap0 = (Map<String, T>) redisArr.get(idx);

...
public class JsonUtil {
	...
	public final static <T> String stringify(T t) {
		String json = "{}";
		try {
			json = JSON.toJSONString(t, SerializerFeature.DisableCircularReferenceDetect, SerializerFeature.UseISO8601DateFormat, SerializerFeature.WriteNonStringKeyAsString);
		} catch (Exception e) {
			Log.error("", e);
		}
		return json;
	...
	public final static <T> byte[] binaryify(T t) {
		byte[] json = null;
		try {
			json = JSON.toJSONBytes(t, SerializerFeature.DisableCircularReferenceDetect, SerializerFeature.UseISO8601DateFormat, SerializerFeature.WriteNonStringKeyAsString);
		} catch (Exception e) {
			Log.error("", e);
		}
		return json;

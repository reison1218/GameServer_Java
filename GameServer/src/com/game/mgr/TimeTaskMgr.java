...
public final class TimeTaskMgr {
	...
	public final static boolean init() {
		...
		// =======扫描类逻辑start================///
		scanTimer.schedule(new CheckUserUnloadDataTask(), MINUTE * 1, MINUTE * 2);
		// =======重置类逻辑start================///
		try {
			resetTimer = new Timer("resetTimeer");
			// 0点重置定时任务
			resetTimer.scheduleAtFixedRate(new ResetUserDataTask(), TimeUtil.getNextDate(), MINUTE * 60 * 24);
...
class CheckUserUnloadDataTask extends Task {

	public CheckUserUnloadDataTask() {
	}
	...
	public void exec() {
		...
		GameMgr.checkUnloadData();
	}

}

class ResetUserDataTask extends Task {

	public ResetUserDataTask() {
	}
	...
	public void exec() {
		try {
			// 设置为重置状态
			GameMgr.server_status = ServerStatus.RESETTING;
